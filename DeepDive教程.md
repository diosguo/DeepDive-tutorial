...
DeepDive supports categorical variables, which take integer values ranging from 0 to a user-specified upper bound. The variable relation is declared similarly as a Boolean variable except that the declaration is followed by a `Categorical(N)` where `N` is the number of categories the variables can take, defining the size of the domain. Each variable can take values from 0, 1, ..., `N`-1. For instance, in the [chunking example](http://deepdive.stanford.edu/example-chunking), a categorical variable of 13 possible categories is declared as follows:
...
​	All subsequent `deepdive do` commands will run what has been compiled, so an app has to be compiled at least once initially before [executing any part of it](http://deepdive.stanford.edu/ops-execution).
...
- When to compile
  ...
  The following files in [a DeepDive application](http://deepdive.stanford.edu/deepdiveapp) are considered as the input to `deepdive compile`:
...
- Compile-time checks
  ...
  - `compiled_base_relations_have_input_data`: all the tables declared that are not filled by an extractor must have input data in `input/` that can be [loaded automatically](http://deepdive.stanford.edu/ops-data#organizing-input-data).
...
DeepDive provides a handful of commands and conventions to manage input data to an application as well as the data produced by it. The actual processing of the data is discussed in a page about [executing the DeepDive application](http://deepdive.stanford.edu/ops-execution).
...
The database for the DeepDive application is configured through [the `db.url` file](http://deepdive.stanford.edu/deepdiveapp#db-url) or can be overridden with the `DEEPDIVE_DB_URL`environment variable.
...
To unload data produced for a relation to files, such as *bar* into two sinks `dump-1.tsv` and `/data/dump-2.csv.bz2`, assuming you have the table populated in the database by [executing some data processing](http://deepdive.stanford.edu/ops-execution), run:
...
2. Run all processes that correspond to the [normal derivation rules](http://deepdive.stanford.edu/writing-dataflow-ddlog) as well as the [rules with user-defined functions](http://deepdive.stanford.edu/writing-udf-python).
3. Ground the factor graph according to the [inference rules defining the model for statistical inference](http://deepdive.stanford.edu/writing-model-ddlog).
4. Perform [weight learning and inference](http://deepdive.stanford.edu/ops-model) to compute marginal probability of every variable.
5. Generate [calibration plots and data](http://deepdive.stanford.edu/calibration) for debugging.
...
Refer to the [next section](http://deepdive.stanford.edu/ops-execution#compiled-processes-and-data-flow) for more detail about these TARGET names.
...
DeepDive compiles a few built-in processes into the data flow graph that are necessary for initialization, statistical learning and inference, and calibration. The rest of the processes correspond to the rules for deriving relations according to the [DDlog program](http://deepdive.stanford.edu/writing-dataflow-ddlog) and the [extractors in deepdive.conf](http://deepdive.stanford.edu/configuration#extraction-and-extractors). Below is a data flow graph compiled for the [tutorial example](http://deepdive.stanford.edu/example-spouse), which can be found at `run/dataflow.svg` for any application.[![img](http://deepdive.stanford.edu/images/spouse/dataflow.svg)](http://deepdive.stanford.edu/images/spouse/dataflow.svg)
...
- `process/init/relation/*R*`
  ...
  Such process [creates the table *R* in the database and loads data from `input/*R*.*`](http://deepdive.stanford.edu/ops-data). These processes are automatically created and added to the data flow for every relation that is not derived from any other relation nor output by any process.

- `process/grounding/*`

  All processes for [grounding the factor graph](http://deepdive.stanford.edu/ops-model) are put under this namespace.

- `process/model/*`

  All processes for [learning and inference](http://deepdive.stanford.edu/ops-model) are put under this namespace.
...
- `data/model/*`
  ...
  All data relevant to statistical inference that go into the database are put under this namespace. For example, `data/model/weights`and `data/model/probabilities` correspond to [tables and views that keep the learning and inference results](http://deepdive.stanford.edu/reserved_tables).
...
For every DeepDive application, [executing any data processing it defines](http://deepdive.stanford.edu/ops-execution) is ultimately to supply with necessary bits in the construction of the [statistical model declared in DDlog](http://deepdive.stanford.edu/writing-model-ddlog) for [*joint inference*](http://deepdive.stanford.edu/inference). DeepDive provides several commands to streamline operations on the statistical model, including its creation (*grounding*), parameter estimation (*learning*), and computation of probabilities (*inference*) as well as keeping and reusing the parameters of the model (*weights*).

## Getting the inference result

To simply get the inference results, i.e., the marginal probabilities of the [random variables defined in DDlog](http://deepdive.stanford.edu/writing-model-ddlog#variable-declarations), use the following command:
...
To better understand the inference result for debugging, please refer to the pages about [calibration](http://deepdive.stanford.edu/calibration), [Dashboard](http://deepdive.stanford.edu/dashboard), [labeling](http://deepdive.stanford.edu/labeling), and [browsing data](http://deepdive.stanford.edu/browsing).
...
The [inference rules written in DDlog](http://deepdive.stanford.edu/writing-model-ddlog#inference-rules) give rise to a data structure called *factor graph* DeepDive uses to perform statistical inference.*Grounding* is the process of materializing the factor graph as a set of files by laying down all of its [variables and factors in a particular format](http://deepdive.stanford.edu/factor_graph_schema). This process can be performed using the following command:
...
The above can be viewed as a shorthand for executing the following [built-in processes](http://deepdive.stanford.edu/ops-execution#built-in-data-flow-nodes):
...
DeepDive learns the weights of the grounded factor graph, i.e., estimates the maximum likelihood parameters of the statistical model from the variables that were assigned labels via [distant supervision rules written in DDlog](http://deepdive.stanford.edu/writing-model-ddlog#scoping-and-supervision-rules). DimmWitted inference engine uses *Gibbs sampling*with *stochastic gradient descent* to learn the weights.
...
Suppose a Python UDF at `udf/fn.py` imports `deepdive` and [`ddlib`](http://deepdive.stanford.edu/gen_feats) as suggested in the [guide for writing UDFs](http://deepdive.stanford.edu/writing-udf-python). When run normally as a Python script, it will give an error that looks like this:
